...
function parseBrief(text) {
  ...
  SECTION_LABELS.forEach((label, idx) => {
    ...
    const nextLabel = SECTION_LABELS.slice(idx + 1).find(l => text.indexOf(l) > start);
...
function BriefView({ brief, meta, onBack }) {
  ...
  return (
    <div className="brief-view">
      ...
      <div className="brief-chip-row">
        <span className="chip">{meta.meetingType}</span>
...
export default function Home() {
  ...
  const [form, setForm] = useState({ meetingType: "", whoTheyAre: "", desiredOutcome: "" });
    ...
    <>
      ...
      <div className="app">
        ...
        <main>
              ...
              <form onSubmit={handleSubmit}>
                <div className="field">
                  ...
                  <div className="select-wrap">
                    <select value={form.meetingType} onChange={e => setForm({ ...form, meetingType: e.target.value })} required>
                ...
                <div className="field">
                  ...
                  <input type="text" placeholder="Name, company, role" value={form.whoTheyAre} onChange={e => setForm({ ...form, whoTheyAre: e.target.value })} required />
                  <div className="field-hint">We'll research them automatically.</div>
                </div>
                <div className="field">
                  <label>What you want from this meeting</label>
                  <textarea placeholder="The specific outcome you're walking in for" value={form.desiredOutcome} onChange={e => setForm({ ...form, desiredOutcome: e.target.value })} required rows={3} />
                </div>
          ...
          {view === "brief" && result && (
            <div className="fade-in">
              <BriefView brief={result.brief} meta={result} onBack={() => setView("form")} />
            </div>
          )}
              ...
              {history.length > 0 && (
                <>
                  <div className="history-stats">
                    <div className="stat-box"><div className="stat-num">{history.length}</div><div className="stat-label">Briefs</div></div>
                    <div className="stat-box"><div className="stat-num">{[...new Set(history.map(h => h.meetingType.split(" — ")[0]))].length}</div><div className="stat-label">Types</div></div>
                    <div className="stat-box"><div className="stat-num">{history.filter(h => h.meetingType.includes("Investor")).length}</div><div className="stat-label">Investor</div></div>
                  </div>
                  <div className="history-list">
                    {history.map(item => <HistoryItem key={item.id} meeting={item} onClick={() => { setResult(item); setView("brief"); }} />)}
                  </div>
                </>
              )}
            </div>
      ...
      <style jsx global>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        header { position: sticky; top: 0; z-index: 100; background: var(--bg); border-bottom: 1px solid var(--border); padding: 0 20px; }
        ...
        main { flex: 1; padding: 32px 20px 80px; }
        ...
        .field-hint { font-size: 0.62rem; color: var(--accent); opacity: 0.7; margin-top: 6px; }
        ...
        select option { background: #1a1a1a; color: var(--ink); }
        ...
        .loading-wrap { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 50dvh; gap: 24px; }
        ...
        .chip { font-size: 0.62rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--accent); opacity: 0.8; }
        ...
        .history-item { width: 100%; background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); padding: 16px; cursor: pointer; text-align: left; -webkit-tap-highlight-color: transparent; }
        ...
        @media print { body { background: white; color: black; } header, .back-btn, .print-btn { display: none; } .section { border-left-color: #ccc; } .section-label { color: #666; } }
      `}</style>
    </>
  );
}
